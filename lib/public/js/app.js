...
FMApp.controller('FileManagerCtrl', [
    '$scope',
    '$http',
    '$location',
    function ($scope, $http, $location) {
        ...
        var disabledSelectFileNameList = ['DevOps_Artifacts'];
        var setCurFiles = function (relPath) {
            $http
                .get('api' + relPath)
                .success(function (files) {
                    if (relPath.indexOf(disabledSelectFileNameList[0]) > -1) {
                        files.sort(function (c, n) {
                            return n.mtime - c.mtime;
                        });
                    }
                    files.forEach(function (file) {
                        file.relPath = relPath + encodeURIComponent(file.name);
                        if (file.isFolder) file.relPath += '/';
                        file.selected = false;
                        file.selectable = disabledSelectFileNameList.indexOf(file.name) == -1;
                        file.humanSize = humanSize(file.size);
                        file.humanTime = humanTime(file.mtime);
                        file.link = window.location.origin + '/api' + file.relPath;
                    });
                    FM.curFiles = files;
        ...
        $scope.$watch('FM.warnData', function () {
            if (FM.warnData === '__init__') return;
            $('#successAlert').hide();
            $('#errorAlert').hide();
        ...
        function httpRequest(method, url, params, data, config) {
            var conf = {
                method: method,
                url: url,
                params: params,
        ...
        FM.copyFileLink = function (file) {
            var copyTextInput = document.createElement('input');
            document.body.appendChild(copyTextInput);
            copyTextInput.style.height = '0';
            copyTextInput.style.boxShadow = 'none';
            copyTextInput.style.border = 'none';
            copyTextInput.style.color = 'transparent';
            copyTextInput.value = file.link;
            copyTextInput.select();
            copyTextInput.setSelectionRange(0, 9999999);
            try {
                var successful = document.execCommand('copy');
                var msg = successful ? 'successful' : 'unsuccessful';
                console.log('Copying path command was ' + msg + ': ' + copyTextInput.value);

                FM.successData = 'File path copied: ' + copyTextInput.value;
            } catch (err) {
                console.log('Oops, unable to copy path');
            }
            ...
            switch (btnName) {
                case 'download':
                    if (FM.selection.length === 0) return true;
                        ...
                        for (var i in FM.selection) {
                            if (FM.selection[i].isFolder) return true;
                        }
                        return false;
                    }
                case 'delete':
